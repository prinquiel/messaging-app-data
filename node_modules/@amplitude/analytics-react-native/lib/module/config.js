import { Config, MemoryStorage, UUID, CookieStorage, getCookieName, getQueryParams, FetchTransport } from '@amplitude/analytics-core';
...
export const useReactNativeConfig = async (apiKey, options) => {
  ...
  (_config$loggerProvide = config.loggerProvider) === null || _config$loggerProvide === void 0 || _config$loggerProvide.log(`Init: storage=${cookieStorage.constructor.name} restoredSessionId = ${(previousCookies === null || previousCookies === void 0 ? void 0 : previousCookies.sessionId) ?? 'undefined'}, optionsSessionId = ${(options === null || options === void 0 ? void 0 : options.sessionId) ?? 'undefined'}`);
  return config;
};
export const createCookieStorage = async (overrides, baseConfig = getDefaultConfig()) => {
  const options = {
    ...baseConfig,
    ...overrides
  };
