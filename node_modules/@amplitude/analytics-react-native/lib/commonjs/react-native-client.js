...
var _campaignTracker = require("./campaign/campaign-tracker");
...
class AmplitudeReactNative extends _analyticsCore.AmplitudeCore {
  ...
  async _init(options) {
    ...
    // identity store.
    const connector = (0, _analyticsCore.getAnalyticsConnector)();
    ...
    await this.add(new _analyticsCore.IdentityEventSender()).promise;
    ...
    (_this$config$loggerPr = this.config.loggerProvider) === null || _this$config$loggerPr === void 0 || _this$config$loggerPr.log(`Init: startNewSessionIfNeeded = ${isNewSession ? 'yes' : 'no'}, sessionId = ${this.getSessionId() ?? 'undefined'}`);
  ...
  shutdown() {
    ...
    (_this$appStateChangeH = this.appStateChangeHandler) === null || _this$appStateChangeH === void 0 || _this$appStateChangeH.remove();
  }
  async runAttributionStrategy(attributionConfig, isNewSession = false) {
    if ((0, _platform.isNative)()) {
      return;
    }
    const track = (...args) => this.track(...args).promise;
    ...
    const campaignTracker = new _campaignTracker.CampaignTracker(this.config.apiKey, {
      ...attributionConfig,
      storage,
      track,
      onNewCampaign
  ...
  setUserId(userId) {
    ...
    (0, _analyticsCore.setConnectorUserId)(userId);
  }
  getDeviceId() {
    var _this$config2;
  ...
  setDeviceId(deviceId) {
    ...
    (0, _analyticsCore.setConnectorDeviceId)(deviceId);
  }
  ...
  getOptOut() {
    var _this$config4;
    return (_this$config4 = this.config) === null || _this$config4 === void 0 ? void 0 : _this$config4.optOut;
  }
  ...
  setSessionIdInternal(sessionId, eventTime) {
    ...
    if (this.config.trackingSessionEvents) {
      ...
      (_this$config$loggerPr2 = this.config.loggerProvider) === null || _this$config$loggerPr2 === void 0 || _this$config$loggerPr2.log(`SESSION_END event: previousSessionId = ${previousSessionId ?? 'undefined'}`);
      ...
      (_this$config$loggerPr3 = this.config.loggerProvider) === null || _this$config$loggerPr3 === void 0 || _this$config$loggerPr3.log(`SESSION_START event: sessionId = ${sessionId}`);
  ...
  async process(event) {
    ...
    // Set user properties
    if (event.event_type === _analyticsCore.SpecialEventType.IDENTIFY && event.user_properties) {
  ...
  enterForeground(timestamp) {
    ...
    (_this$config$loggerPr4 = this.config.loggerProvider) === null || _this$config$loggerPr4 === void 0 || _this$config$loggerPr4.log('App Activated');
...
var _default = exports.default = createInstance();
